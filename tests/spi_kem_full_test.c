...
int main(int argc, char *argv[])
{
    ...
    if (strncasecmp(argv[1], "kyber_512", 9) == 0)
    {
        ...
        spi_conf.bits_per_word = SPI_BITS_PER_WORD;
        spi_conf.speed = SPI_MAX_SPEED_HZ;
        spi_conf.lsb = SPI_LSB_FIRST;
        spi_conf.name = spi_dev_name;
        ec = IPQP_config(IPQP_PROV_SPI_ITRI, &spi_conf);
        if (ec != IPQP_EC_SUCCESS)
        {
            printf("ERROR: Failed to config IPQP. (%d)\r\n", ec);
            FAIL_n_EXIT;
