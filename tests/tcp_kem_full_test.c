...
int main(int argc, char *argv[])
{
    ...
    if (strncasecmp(argv[1], "kyber_512", 9) == 0)
    {
        ...
        IPQP_ErrorCode ec = IPQP_config(IPQP_PROV_TCP_ITRI, &tcp_conf);
        ...
        tcp_conf.fd = -1;
        tcp_conf.ip = ip_addr;
        tcp_conf.port = TCP_DEV_PORT;
        tcp_conf.timeout_sec = TCP_TIMEOUT_SEC;
        tcp_conf.retries_on_failure = TCP_RETRIES_ON_FAILURE;

        ec = IPQP_config(IPQP_PROV_TCP_ITRI, &tcp_conf);
        if (ec != IPQP_EC_SUCCESS)
        {
            printf("ERROR: Failed to config IPQP. (%d)\r\n", ec);
            FAIL_n_EXIT;
