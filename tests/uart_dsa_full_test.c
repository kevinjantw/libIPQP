...
int main(int argc, char *argv[])
{
    ...
    if (strncasecmp(argv[1], "dilithium_2", 11) == 0)
    {
        ...
        IPQP_ErrorCode ec = IPQP_config(IPQP_PROV_UART_ITRI, &uart_conf);
        ...
        /* Config IPQP */
        uart_conf.fd = -1;
        uart_conf.name = uart_dev_name;
        uart_conf.open_flags = UART_OPEN_FLAGS;
        uart_conf.input_mode_flags = UART_INPUT_MODE_FLAGS;
        uart_conf.output_mode_flags = UART_OUTPUT_MODE_FLAGS;
        uart_conf.control_mode_flags = UART_CONTROL_MODE_FLAGS;
        uart_conf.local_mode_flags = UART_LOCAL_MODE_FLAGS;
